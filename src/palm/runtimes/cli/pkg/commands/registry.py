...
from palm.runtimes.cli.pkg import actions
from palm.runtimes.cli.pkg.context import CliContext
from palm.runtimes.cli.pkg.display import (
    render_definition_catalog,
    render_instance_table,
    render_job_status,
)
from palm.runtimes.cli.pkg.doctor import run_doctor
from palm.runtimes.cli.pkg.instance_ops import (
    filter_summaries,
    parse_instance_list_flags,
    prune_terminal_instances,
)
from palm.runtimes.cli.pkg.output import emit_json, snapshots_to_json, summaries_payload
...
def _cmd_status(ctx: CliContext, args: list[str]) -> int:
    ...
    if ctx.output_format == "json":
        from palm.runtimes.cli.pkg.job_context import inspect_job_json
...
def _cmd_flow_list(ctx: CliContext, _args: list[str]) -> int:
    ...
    for flow in flows:
        ...
        from palm.runtimes.cli.pkg.display import flow_detail_label
