...
def _build_input_schema(
    ...
    *,
    ...
    if status not in {"waiting", "running"}:
        # Still expose schema on waiting; skip pure terminal
    ...
    # Non-interactive resource/transform steps: schema may still note kind
    ...
    if not field_type and not choices and step_kind not in {"collection", "summary", "commit"}:
        # Fallback text when waiting without field_type
        if status == "waiting":
            field_type = "text"
        else:
            return None
