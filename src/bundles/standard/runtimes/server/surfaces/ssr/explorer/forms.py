...
from plugins.kits.server.ssr.render import escape
from palm.core.context.state_schema import DictStateSchema
from bundles.standard.runtimes.server.surfaces.ssr.explorer.schemas import build_flow_submit_schema
...
def _render_flow_select(flows: list[FlowDefinition], *, selected: str) -> str:
    from bundles.standard.runtimes.server.surfaces.ssr.explorer.pages.utils import flow_option_label
...
def _flow_context_panel(flow: FlowDefinition) -> str:
    from bundles.standard.runtimes.server.surfaces.ssr.explorer.components import badge
    from bundles.standard.runtimes.server.surfaces.ssr.explorer.pages.utils import flow_description
...
def resource_invoke_form(
    ...
    *,
    ...
    if result is not None:
        from bundles.standard.runtimes.server.surfaces.ssr.explorer.components import code_block
...
def wizard_input_form(
    ...
    *,
    ...
    if prompt.get("step_kind") == "collection" or prompt.get("collection_phase"):
        from bundles.standard.runtimes.server.surfaces.ssr.explorer.components import collection_form
...
def collection_remove_form(instance_id: str, prompt: Mapping[str, Any]) -> str:
    ...
    from bundles.standard.runtimes.server.surfaces.ssr.explorer.components import (
...
def collection_select_form(instance_id: str, prompt: Mapping[str, Any]) -> str:
    ...
    from bundles.standard.runtimes.server.surfaces.ssr.explorer.components import collection_list
