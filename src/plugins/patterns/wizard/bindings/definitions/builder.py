...
from plugins.patterns.wizard.flow.extensions.registry import default_wizard_step_registry
from plugins.patterns.wizard.flow.phases.resource import default_resource_prompt
from plugins.patterns.wizard.flow.phases.transform import default_transform_prompt
from plugins.patterns.wizard.flow.validation import StepValidationRule
...
def _step_from_mapping(data: dict[str, Any]) -> WizardStepConfig:
    ...
    if step_kind == "collection":
        from plugins.patterns.wizard.flow.collection.config import item_fields_from_mapping
