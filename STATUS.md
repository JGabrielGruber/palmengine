...
**Last Updated:** September 19, 2026 · José opened **0.70** (2026-09-19). Plan `0.70.0`. Adapter `0.70.1` (`palm.kits.authoring`). ADR [038](docs/adr/038-authoring-adapter.md) **Proposed**. Package stamp stays `0.68.0`. Assist stays until [surface deflation](docs/vision/VISION-SURFACE-DEFLATION.md).  
...
José opened 0.70 (2026-09-19). Package **`palm.kits.authoring`**. Adapter `0.70.1`: `land(host)` / `commit(body)` on embedded `definitions`. Handle class unnamed. Do not stamp `INTENTION_KITS`. Do not grow land verbs on present. Do not require Design on `embedded()`. Do not compost Assist. Do not pay [SD-024](TECH-DEBT.md#sd-024).
...
| **Next** | `0.70.2` present starts the committed definition. Pack / snapshot / handle class named as later slices. |
