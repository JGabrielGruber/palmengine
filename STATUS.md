...
**Last Updated:** September 17, 2026 · **0.69.3** landed: spawn without nested park (`FlowExecutionService.spawn_sibling`). Guidance stays operator-wait. Nested park leftover stays. Package stamp stays `0.68.0`. ADR [037](docs/adr/037-navigator-invert.md) **Proposed**. Next: `0.69.4` `palm.kits.present`. Assist stays until [surface deflation](docs/vision/VISION-SURFACE-DEFLATION.md).  
...
`0.69.3` shipped: `FlowExecutionService.spawn_sibling` starts a same-session sibling (no `session_id` on the job, then `attach_after_start`). Parent stays `WAITING_FOR_INPUT`. No `WaitInterest` on the sibling. Leftover `until_input` nested park stays. Do not compost Assist. Do not pay [SD-024](TECH-DEBT.md#sd-024).
