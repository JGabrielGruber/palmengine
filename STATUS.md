...
**Last Updated:** September 17, 2026 · **0.69.5** landed: present kit owns `guidance_definition_id`; stamp caller + replace predicate. Package stamp stays `0.68.0`. ADR [037](docs/adr/037-navigator-invert.md) **Proposed**. Next: `0.69.6` new wizard pack. Assist stays until [surface deflation](docs/vision/VISION-SURFACE-DEFLATION.md).  
...
`0.69.5` shipped: present kit owns `guidance_definition_id` (`str | None`). Unset → no empty-handed start. Stamp after attach iff started definition id equals that key. Replace iff attached and definition id equals. Do not flatten onto `PalmSettings`. Do not compost Assist. Do not pay [SD-024](TECH-DEBT.md#sd-024).
...
| **Next** | `0.69.6` new wizard pack beside `operator_entry` |
