...
**Last Updated:** September 17, 2026 · **0.69.4** landed: `palm.kits.present` kit-as-composition (bind / present / submit / start / attach / focus). Handle class unnamed. Package stamp stays `0.68.0`. ADR [037](docs/adr/037-navigator-invert.md) **Proposed**. Next: `0.69.5` `guidance_definition_id` + stamp caller. Assist stays until [surface deflation](docs/vision/VISION-SURFACE-DEFLATION.md).  
...
`0.69.4` shipped: `palm.kits.present` holds one `BoundSurface` and walks bind / present / submit / start / attach / focus. Present is `JobInspectable` + wait plane. Start reuses `spawn_sibling`. Do not stamp. Do not seed `guidance_definition_id`. Do not compost Assist. Do not pay [SD-024](TECH-DEBT.md#sd-024).
...
| **Next** | `0.69.5` kit-contributed `guidance_definition_id` + stamp caller |
