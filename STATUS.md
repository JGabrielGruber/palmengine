...
**Last Updated:** September 17, 2026 · **0.69.7** landed: empty-handed start dogfood on embedded (floor proof). Package stamp stays `0.68.0`. ADR [037](docs/adr/037-navigator-invert.md) **Proposed**. Theme stays open — **José** exits. Assist stays until [surface deflation](docs/vision/VISION-SURFACE-DEFLATION.md).  
...
`0.69.7` shipped: one embedded walk without Assist. Bind → empty-handed `start()` of **`navigator`** (kit `guidance_definition_id` after bind) → stamp → stay WAITING → sibling start (no job `session_id`) → `focus(guidance_instance_id)` home. Tests `tests/test_navigator_dogfood_0_69_7.py`. Do not compost Assist. Do not pay [SD-024](TECH-DEBT.md#sd-024). ADR-037 stays **Proposed**.
...
| **Next** | José exit judgment (theme open; ADR-037 Proposed) |
