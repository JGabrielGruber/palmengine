...
**Last Updated:** September 20, 2026 · José opened **0.71**. Floor: **adopt** a named place. ADR [039](docs/adr/039-place-registry-adopt.md) **Proposed**. `0.71.1` done: `WorkloadEngine.adopt`, `adopt:` prefix on combined spawn, fail closed, 11 tests. Package stamp stays `0.68.0`. Spawn via `workload:` stays [0.63.16](docs/vision/closed/VISION-0.63.md). Assist stays until [surface deflation](docs/vision/VISION-SURFACE-DEFLATION.md).  
...
José opened 0.71 (2026-09-20). `0.71.1` done: `WorkloadEngine.adopt` records a named existing body as READY without calling a runner. `adopt:` prefix registered on `combined_structure_spawn_port`. Structure `ENSURE_PLACE` converges. Fail closed on empty id or missing handle. Release is unbind. 11 tests in `tests/test_place_adopt_0_71_1.py`. Do not reopen 0.56. Do not spawn as the floor (`workload:` already `0.63.16`). Do not implement Tiny LLM. Do not compost Assist. Do not add a `palm` `create_flow` `if` ([SD-025](TECH-DEBT.md#sd-025)). Working prefix `adopt:` until José locks.
...
| **Next** | `0.71.2+` growth (projection, release-as-unbind engine-side). |
...
| Slice | Intent | Status |
|-------|--------|--------|
| **0.71.0** | Plan: VISION, ADR Proposed, STATUS, PALM pointer | done |
| **0.71.1** | Adopt into the workload book; structure ENSURE; fail closed | **done** |
| **0.71.2+** | Growth: projection; release-as-unbind engine-side | queued |
