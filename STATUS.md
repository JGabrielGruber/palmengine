...
José opened 0.71 (2026-09-20). Plan `0.71.0`. Floor `0.71.1` adopt. Growth `0.71.2` projection. Compost `0.71.3` extra maps. Invert `0.71.4` typed book binds. Invert `0.71.5` typed `Workload` / `WorkloadHandle` book-row reads. Invert `0.71.6` typed `host_bind` (shell / engine / effects / spawn hands). Invert `0.71.7` typed `place_registry.engine_from_spawn` (`RegisteredPlaceSpawn`, no Protocol isinstance). Invert `0.71.8` typed adopt payload handle (no dict | `base_url` coercion). Compost `0.71.9` overlay (bare / `os:` no second map beside the book). **All landed.** José locked names (2026-09-20): method `adopt`, prefix `adopt:`, empty `runtime` on adopted rows, adopted rows stay `Workload`. Leftovers stay **named residual**: `RegisteredPlaceSpawn.handles` (place-id / `os:` stash, not book bind), StructureEngine place observations (assemble/admission; bare ready when book-bound), remaining `isinstance` on `workload_place` (env dict ~L198; typed `WorkloadHandle` accept ~L341) and `WorkloadEngine` (runtime bind ~L71/L73; argv-must-not-be-str ~L273). Do not reopen 0.56. Do not spawn as the floor (`workload:` already `0.63.16`). Do not implement Tiny LLM. Do not compost Assist. Do not add a `palm` `create_flow` `if` ([SD-025](TECH-DEBT.md#sd-025)). **Next:** José exit judgment. No queued further compost. ADR-039 stays **Proposed**. Theme stays **open**.
