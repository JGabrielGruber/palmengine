...
**Last Updated:** September 17, 2026 · **0.69.2** landed: session-side attach after start (`SessionService.attach_after_start`). Job stays session-ignorant. Package stamp stays `0.68.0`. ADR [037](docs/adr/037-navigator-invert.md) **Proposed**. Next: `0.69.3` spawn without nested park. Assist stays until [surface deflation](docs/vision/VISION-SURFACE-DEFLATION.md).  
...
`0.69.2` shipped: `SessionService.attach_after_start` after execution start. Do not copy `session_id` onto the job. Attach does not stamp. Leftover `SessionOwnershipHook` stays. Do not compost Assist. Do not pay [SD-024](TECH-DEBT.md#sd-024).
...
| **Next** | `0.69.3` spawn without nested park |
