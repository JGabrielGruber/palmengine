...
| **Capability (structure / organ)** | Named system organ on `StructureDefinition.capabilities` (journal, outbox, work_drain, …) — machine organs, not product domains. Structure fact: listed → hand installs → it runs. Omitted → it does not run. | Product feature flag marketing; a second `may_run_business`; `CompositionProfile.capabilities` |
| **Capability (composition / workload)** | `CompositionProfile.capabilities` — **workloads** plane flag only @ HEAD. Different catalog from structure organs. Same English word; keep catalogs separate. | Organ names (`outbox`, …) on composition; package membership |
...
| **Composition** | Phenotype membership declared on a process (`CompositionProfile`: services · surfaces · workloads). Plugin packages stay latch (`INSTALLED_*`) until composition-owned package install lands. Organs stay structure. | Docker Compose the product; fusing latch with phenotype |
| **Install · enable · SoT (posture verbs)** | **install** = who puts a member on the process (phenotype: composition; packages @ HEAD: latch; organs: structure hands). **enable** = structure gates organs (+places/refuse/admission); package enable under reading A is **future** after composition installs. **SoT after load** holds for `StructureDefinition`; **unpaid** for plugin packages. | Collapsing install≈enable; claiming package SoT already StructureDefinition-grade |
