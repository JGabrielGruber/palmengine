...
| `palm.kits` | Surface kits (`server`, **`present`**, **`authoring`**, …). **`palm.kits.present`** as-built `0.69.4` (**turn invert** + **kit-as-composition**); `0.69.5` owns **`guidance_definition_id`** and is the stamp/replace caller. **`palm.kits.authoring`** as-built `0.70.1` (`land` / `commit` on `host.definitions`); `0.70.5` job leaf via resource that walks `bound()`; `0.70.6` `commit` lands `kind: resource`; `0.70.7` `bound()` from the started host. Handle classes unnamed. Theme: [VISION-0.70](vision/VISION-0.70.md) (**open**). Seed: [VISION-AUTHORING](vision/VISION-AUTHORING.md). Present: [VISION-0.69](vision/closed/VISION-0.69.md) (**closed**). | SD-011 ✅ · SD-025 |
...
| Theme plan | **open** [VISION-0.70](vision/VISION-0.70.md) · ADR [038](adr/038-authoring-adapter.md) **Proposed** · seed [VISION-AUTHORING](vision/VISION-AUTHORING.md) · closed [VISION-0.69](vision/closed/VISION-0.69.md) |
...
| Authoring (**0.70 open**) | [VISION-0.70](vision/VISION-0.70.md) · seed [VISION-AUTHORING](vision/VISION-AUTHORING.md) — land a shape; authoring adapter + definition pack; ADR [038](adr/038-authoring-adapter.md) **Proposed** |
