...
| Place registry (**0.71 open**) | [VISION-0.71](vision/VISION-0.71.md) — `0.71.0`–`0.71.9` landed (adopt + projection + map/overlay compost + typed binds + typed adopt payload); names locked (`adopt` / `adopt:` / empty `runtime` / `Workload` rows); residual named (handles / assemble observations / remaining isinstance gates); José exit next; ADR [039](adr/039-place-registry-adopt.md) **Proposed**; spawn `workload:` already `0.63.16` |
