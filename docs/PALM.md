...
| Theme plan | **open 0.71** [VISION-0.71](vision/VISION-0.71.md) (`0.71.0`–`0.71.7` landed; names locked; José exit next) · ADR [039](adr/039-place-registry-adopt.md) **Proposed** · scout [VISION-0.56](vision/VISION-0.56.md) · closed [VISION-0.70](vision/closed/VISION-0.70.md) · ADR [038](adr/038-authoring-adapter.md) **Accepted** |
...
| Place registry (**0.71 open**) | [VISION-0.71](vision/VISION-0.71.md) — `0.71.0`–`0.71.7` landed (adopt + projection + map compost + typed binds); names locked (`adopt` / `adopt:` / empty `runtime` / `Workload` rows); residual named (handles / assemble observations / overlay / EffectIntent coercion); José exit next; ADR [039](adr/039-place-registry-adopt.md) **Proposed**; spawn `workload:` already `0.63.16` |
...
| Workload plane (place registry) | **Scout** (0.56) engine landed · **open 0.71** adopt through typed spawn bind (`0.71.0`–`0.71.7`); names locked; residual named; José exit next — [VISION-0.71](vision/VISION-0.71.md) · ADR [039](adr/039-place-registry-adopt.md) **Proposed** |
